...
let game_util = require('./game_util'),
 ...
 */
let AiStrategy = {
    RandomOpenCell: "RandomOpenCell",
    Minimax: "Minimax",
    AlphaBetaPruning: "AlphaBetaPruning"
};

// The strategy to use for the AI turn.
let ACTIVE_AI_STRATEGY = AiStrategy.AlphaBetaPruning;
...
function doRandomOpenCell(boardInfo) {
    ...
    for (let y = 0; y < boardInfo.rowCount; y++) {
        for (let x = 0; x < boardInfo.colCount; x++) {
            if (game_util.canPlaceMarker(x, y, boardInfo.cells)) {
...
function doMinimaxImpl(boardInfo, myMarker, operation, depth) {
    ...
    for (let i = 0; i < moveList.length; i++) {
        ...
        if (operation === Operation.Max) {
            ...
            // Change the board state.
            boardInfo.cells[cell.x][cell.y] = game_util.getOppositeMarker(myMarker);
...
function generateMoves(boardInfo) {
    ...
    if (game_util.checkWin(boardInfo.cells, game_util.Marker.X) ||
        game_util.checkWin(boardInfo.cells, game_util.Marker.O)) {
        return moveList;
    }
...
function evaluateScore(boardInfo, myMarker) {
    ...
    if (game_util.checkWin(boardInfo.cells, myMarker)) {
        return 1;
    } else if (game_util.checkWin(boardInfo.cells, game_util.getOppositeMarker(myMarker))) {
        return -1;
    } else {
        return 0;
    }
...
function getMove(boardInfo, myMarker) {
    switch (ACTIVE_AI_STRATEGY) {
        ...
        default:
            console.error('Unknown Ai strategy: ' + ACTIVE_AI_STRATEGY);
    }
}
...
module.exports = {
    ...
    setStrategy: function(strategy) {
        console.log('Setting AI strategy to: ' + strategy);
        ACTIVE_AI_STRATEGY = strategy;
    },
